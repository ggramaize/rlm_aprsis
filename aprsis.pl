#!/usr/bin/perl
#
# Freeradius APRS-IS passcode generator
# Geoffroy GRAMAIZE <geoffroy+dev@gramaize.eu>
#

use strict;
use warnings;

# use ...
use Data::Dumper;

# Bring the global hashes into the package scope
our (%RAD_REQUEST, %RAD_REPLY, %RAD_CHECK, %RAD_STATE);

# Remap return values
use constant {
	RLM_MODULE_REJECT   => 0, # immediately reject the request
	RLM_MODULE_OK       => 2, # the module is OK, continue
	RLM_MODULE_HANDLED  => 3, # the module handled the request, so stop
	RLM_MODULE_INVALID  => 4, # the module considers the request invalid
	RLM_MODULE_USERLOCK => 5, # reject the request (user is locked out)
	RLM_MODULE_NOTFOUND => 6, # user not found
	RLM_MODULE_NOOP     => 7, # module succeeded without doing anything
	RLM_MODULE_UPDATED  => 8, # OK (pairs modified)
	RLM_MODULE_NUMCODES => 9  # How many return codes there are
};

# Remap log severity
use constant {
	L_AUTH         => 2,  # Authentication message
	L_INFO         => 3,  # Informational message
	L_ERR          => 4,  # Error message
	L_WARN         => 5,  # Warning
	L_PROXY        => 6,  # Proxy messages
	L_ACCT         => 7,  # Accounting messages
	L_DBG          => 16, # Only displayed when debugging is enabled
	L_DBG_WARN     => 17, # Warning only displayed when debugging is enabled
	L_DBG_ERR      => 18, # Error only displayed when debugging is enabled
	L_DBG_WARN_REQ => 19, # Less severe warning only displayed when debugging is enabled
	L_DBG_ERR_REQ  => 20, # Less severe error only displayed when debugging is enabled
};

sub aprsis_hash {
	my $callsign = shift;
	my $rootCall = uc( (split( '-',$callsign))[0])."\0";

	my $hash = 0x73e2;
	my $i = 0;
	my $callLen = length $rootCall;

	while( $i+1 < $callLen ) {
		$hash ^= ord(substr($rootCall,$i,1)) << 8;
		$hash ^= ord(substr($rootCall,$i+1,1));
		$i += 2;
	}

	return $hash & 0x7FFF;
}

# Function to handle authorize
sub authorize {
	my $userName = $RAD_REQUEST{'User-Name'};
	
	if( $userName eq '' )
	{
		return RLM_MODULE_NOOP;
	}

	&radiusd::radlog( L_DBG, 'Generating Cleartext-Password for User: ' . $userName);
	&radiusd::radlog( L_DBG, 'Cleartext-Password: ' . aprsis_hash($userName));

	$RAD_CHECK{'Cleartext-Password'} = "" . aprsis_hash($userName);

	return RLM_MODULE_UPDATED;
}


# Function stubs
sub authenticate { return RLM_MODULE_NOOP; }
sub preacct { return RLM_MODULE_NOOP; }
sub accounting { return RLM_MODULE_NOOP; }
sub checksimul { return RLM_MODULE_NOOP; }
sub pre_proxy { return RLM_MODULE_NOOP; }
sub post_proxy { return RLM_MODULE_NOOP; }
sub post_auth { return RLM_MODULE_NOOP; }

# Function to handle detach
sub detach {
	&radiusd::radlog(0,"rlm_aprsis::Detaching. Reloading. Done.");
}
